...
namespace BinaryOperatorTree {
    ...
    func execute_tree_chain{range_check_ptr}(
        ...
        let (output, dict_new) = iterate_tree(trees, mem, dict);
    ...
    func iterate_tree{range_check_ptr}(tree: Tree*, mem: felt*, dict: DictAccess*) -> (
        output: felt, dict_new: DictAccess*
    ) {
        alloc_locals;
        ...
        if (left == -1 and right == -1) {
            return (output=value, dict_new=dict);
        }
        if (value == ns_opcodes.ADD) {
            let (value_left, dict_new) = iterate_tree(tree + left * ns_tree.TREE_SIZE, mem, dict);
            let (value_right, dict_new) = iterate_tree(
                tree + right * ns_tree.TREE_SIZE, mem, dict_new
            );
            return (output=value_left + value_right, dict_new=dict_new);
        }
        ...
        if (value == ns_opcodes.DIV) {
            ...
            return (output=q, dict_new=dict_new);
        }
        if (value == ns_opcodes.MOD) {
            ...
            return (output=r, dict_new=dict_new);
        }
        if (value == ns_opcodes.ABS and left == -1) {
            let (value_right, dict_new) = iterate_tree(tree + right * ns_tree.TREE_SIZE, mem, dict);
            let value = abs_value(value_right);
            return (output=value, dict_new=dict_new);
        }
        ...
        if (value == ns_opcodes.POW) {
            ...
            return (output=p, dict_new=dict_new);
        }
        ...
        if (value == ns_opcodes.NOT and left == -1) {
            let (value_right, dict_new) = iterate_tree(tree + right * ns_tree.TREE_SIZE, mem, dict);
            if (value_right == 0) {
                return (output=1, dict_new=dict_new);
            }
            return (output=0, dict_new=dict_new);
        }
        ...
        if (value == ns_opcodes.MEM and left == -1) {
            ...
            with_attr error_message("incorrect input value to mem {value_right}") {
                return (output=[mem + value_right], dict_new=dict_new);
            }
        }
